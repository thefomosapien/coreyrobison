...
import type { SiteSettings, Project, Thought, Reaction, JourneyItem, AboutDetail } from '@/lib/types';
...
async function getData() {
  try {
    ...
    if (!url || !key) {
      return { settings: null, projects: [], thoughts: [], reactions: [], journeyItems: [], aboutDetails: [] };
    }
...
export default async function Home() {
  const { settings, projects, thoughts, reactions, journeyItems, aboutDetails } = await getData();
  ...
  return (
    <div className="min-h-screen relative" style={{ background: '#FDFCFA', color: '#2A2824' }}>
      ...
      <Nav name={settings.name} currentPage="home" />
